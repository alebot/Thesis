...
CONTAINS
    ...
    open(unit=99, file=TRIM(basics%path_results)//Getproname(basics)//'_temp_1D.dat', &
        action="write", status="unknown", form="formatted")
    write(unit=99, fmt='(A15, A15, A15, A15)') 'radius [au]', 'theta [rad]', &
                                                'phi [rad]', 'T_dust [K]'
    ...
    do i_dust=1, dust%n_dust
        do nr_cell=1, grid%n_cell
            ...
            i_a = grid%cell_nr2idx(1, nr_cell)
            i_b = grid%cell_nr2idx(2, nr_cell)
            i_c = grid%cell_nr2idx(3, nr_cell)
            moco(1) = (grid%co_mx_a(i_a) + grid%co_mx_a(i_a - 1)) / 2.0_r2
            moco(2) = (grid%co_mx_b(i_b) + grid%co_mx_b(i_b - 1)) / 2.0_r2
            moco(3) = (grid%co_mx_c(i_c) + grid%co_mx_c(i_c - 1)) / 2.0_r2
            write(unit=99, fmt='(E15.3, E15.3, E15.3, E15.3)') moco(1), moco(2), moco(3), grid%t_dust(nr_cell, i_dust)
        end do
    end do
    close(unit=99)
