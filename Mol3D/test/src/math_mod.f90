...
MODULE math_mod
    ...
    INTERFACE get_expo
        MODULE PROCEDURE get_expo_r1, get_expo_r2
    END INTERFACE
...
CONTAINS
    ...
    ELEMENTAL FUNCTION planck(tem_in, lam_in) result(planck_result)
        ...
        if ( tem_in .gt. 1.0e-12 ) then
            planck_result = c1 / lam_in /lam_in /lam_in/ lam_in /lam_in /      &
                            (get_expo(c2/(tem_in*lam_in))-1.0_r2)
        else
  ...
  ! ---
    ELEMENTAL FUNCTION dB_dT_l(tem_in, lam_in) result(dB_result)
        ...
        if ( tem_in .gt. 1.0e-12 .and. c2/(tem_in*lam_in) .lt. 300.0_r2 ) THEN
            dB_result = c1 / lam_in /lam_in /lam_in/ lam_in /lam_in /lam_in /  &
                        tem_in / tem_in *                                      &
                        get_expo(c2/(tem_in*lam_in))/                               &
                        (get_expo(c2/(tem_in*lam_in))-1.0_r2)**2.0_r2
        else
            dB_result = 0.0_r2
        endif
    ...
    ELEMENTAL FUNCTION get_expo_r1(var_in) result(var_out)
        !----------------------------------------------------------------------!
        real(kind=r1), intent(in)      :: var_in
        real(kind=r1)                  :: var_out
        !----------------------------------------------------------------------!
        
        var_out = exp(min(max_expo_r1,max(min_expo_r1,var_in)))
        
    END FUNCTION
    
    ELEMENTAL FUNCTION get_expo_r2(var_in) result(var_out)
        !----------------------------------------------------------------------!
        real(kind=r2), intent(in)      :: var_in
        real(kind=r2)                  :: var_out
        !----------------------------------------------------------------------!
        
        var_out = exp(min(max_expo_r2,max(min_expo_r2,var_in)))
    ...
    ELEMENTAL FUNCTION planckhz(tem_in, freq_in) result(planck_result)
        ...
        if ( tem_in .gt. 1.0e-12 ) then
            ...
            planck_result = c1 * freq_in * freq_in * freq_in  /                &
                            (get_expo(c2*freq_in/tem_in)-1.0)
...
!!$  end function integ1
  ...
  function arcdis( &
    ...
    ELEMENTAL FUNCTION gauss_arr(lscale, a, velo) RESULT (gaussian_result)
        ...
        gaussian_result = lscale/a * get_expo( -(velo/a)**2 )
